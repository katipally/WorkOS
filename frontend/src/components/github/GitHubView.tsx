...
import type { GHTab } from "@/types/github";
...
export default function GitHubView() {
  ...
  const {
    repos, reposLoading, activeRepo, currentRepo, repoName,
        ...
        <div className="w-56 shrink-0 flex flex-col bg-sidebar border-r overflow-hidden">
          ...
          <div className="p-2 border-b shrink-0 flex items-center gap-1.5">
            {reposLoading ? <Skeleton className="h-8 w-full rounded" /> : (
              <>
                <Select value={activeRepo || "__none__"} onValueChange={setSelectedRepo}>
          ...
          <ScrollArea className="flex-1 min-h-0">
            <div className="p-2 space-y-0.5">
              ...
              {GH_SIDEBAR_TABS.map(({ id, label, icon: Icon }) => (
                <button key={id} onClick={() => { setActiveGHTab(id); if (id === "code") navigateToDir(browsePath); }}
